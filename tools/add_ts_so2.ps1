$cs = "Server=localhost;Database=Focus80E0;Integrated Security=True;Encrypt=False;TrustServerCertificate=True;"
$conn = New-Object System.Data.SqlClient.SqlConnection $cs
$conn.Open()
$cmd = $conn.CreateCommand(); $cmd.CommandTimeout = 60

$cmd.CommandText = @"
SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
WHERE TABLE_NAME='cCore_Vouchers_0' AND (COLUMN_NAME LIKE '%Name%' OR COLUMN_NAME LIKE '%Type%' OR COLUMN_NAME LIKE '%Class%')
ORDER BY ORDINAL_POSITION
"@
$r = $cmd.ExecuteReader()
Write-Output "=== Vouchers cols ==="
while ($r.Read()) { Write-Output $r.GetValue(0) }
$r.Close()

$cmd.CommandText = @"
SELECT TOP 5 * FROM dbo.cCore_Vouchers_0 WHERE iVoucherType = 5634
"@
try {
    $r = $cmd.ExecuteReader()
    $n = $r.FieldCount
    $hdr = @(); for ($i=0;$i -lt $n;$i++) { $hdr += $r.GetName($i) }
    Write-Output ($hdr -join " | ")
    while ($r.Read()) {
        $parts = @(); for ($i=0;$i -lt $n;$i++) {
            if ($r.IsDBNull($i)) { $parts += "" } else {
                $v = [string]$r.GetValue($i); if ($v.Length -gt 80) { $v = $v.Substring(0,80) }
                $parts += $v
            }
        }
        Write-Output ($parts -join " | ")
    }
    $r.Close()
} catch { Write-Output $_.Exception.Message; try { $r.Close() } catch {} }

# Insert TS if missing
$cmd.CommandText = "SELECT COUNT(*) FROM dbo.cCore_ReportTransactionSet_0 WHERE iReportId=70266 AND iVoucherType=5634"
$exists = [int]$cmd.ExecuteScalar()
Write-Output ("existing 5634 on 70266 = {0}" -f $exists)

if ($exists -eq 0) {
    $cmd.CommandText = "SELECT ISNULL(MAX(iTranId),0)+1 FROM dbo.cCore_ReportTransactionSet_0"
    $newId = [int]$cmd.ExecuteScalar()
    $cmd.CommandText = @"
INSERT INTO dbo.cCore_ReportTransactionSet_0
    (iTranId, iReportId, iTranSetId, iVoucherType, iBRS, iDocumentOption, iFilterGroupId)
VALUES
    ($newId, 70266, 14, 5634, 3, 30, NULL)
"@
    $n = $cmd.ExecuteNonQuery()
    Write-Output ("inserted iTranId={0} rows={1}" -f $newId, $n)
} else {
    Write-Output "already present"
}

$cmd.CommandText = @"
SELECT t.iTranId, t.iReportId, t.iTranSetId, t.iVoucherType, t.iBRS, t.iDocumentOption, v.sName
FROM dbo.cCore_ReportTransactionSet_0 t
LEFT JOIN dbo.cCore_Vouchers_0 v ON v.iVoucherType = t.iVoucherType
WHERE t.iReportId = 70266
"@
try {
    $r = $cmd.ExecuteReader()
    $n = $r.FieldCount
    $hdr = @(); for ($i=0;$i -lt $n;$i++) { $hdr += $r.GetName($i) }
    Write-Output ($hdr -join " | ")
    while ($r.Read()) {
        $parts = @(); for ($i=0;$i -lt $n;$i++) {
            if ($r.IsDBNull($i)) { $parts += "" } else { $parts += [string]$r.GetValue($i) }
        }
        Write-Output ($parts -join " | ")
    }
    $r.Close()
} catch { Write-Output $_.Exception.Message }

$conn.Close()
