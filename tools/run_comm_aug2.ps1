...
Write-Output "`n=== qualified rows ==="
$q2 = @"
SELECT [Sales Order], Salesman, [Opportunity Type], [Contract Amount], [Advance Amount], [Paid %], Qualified
FROM ($base) r
WHERE r.Company = N'Atlas Aluminum'
  AND CAST(r.iDate AS int)/65536 = 2026
  AND (CAST(r.iDate AS int)/256) % 256 = 8
...
"@
$cmd = $conn.CreateCommand(); $cmd.CommandTimeout = 180; $cmd.CommandText = $q2
$r = $cmd.ExecuteReader()
while ($r.Read()) {
    $parts = @()
