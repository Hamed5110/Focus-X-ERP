/* ============================================================================
   Monthly Salesman Commission — Aknan Showroom only (iFaTag 2057)
   New Query report. Do not overwrite the combined 70266 layout.
   Customer Name included. Same rules as the combined report.
...
FROM (
    ...
    LEFT JOIN (
        SELECT
            ...
            END) AS TeamMembers,
            CASE
                WHEN SUM(s2.EligibleAmt) >= 80000 THEN 1.50
                WHEN SUM(s2.EligibleAmt) >= 70000 THEN 1.20
                WHEN SUM(s2.EligibleAmt) >= 50000 THEN 1.00
                ELSE 0
            END AS TeamRatePct,
            CAST(SUM(s2.EligibleAmt) *
                CASE
                    WHEN SUM(s2.EligibleAmt) >= 80000 THEN 0.015
                    WHEN SUM(s2.EligibleAmt) >= 70000 THEN 0.012
                    WHEN SUM(s2.EligibleAmt) >= 50000 THEN 0.010
                    ELSE 0
                END
